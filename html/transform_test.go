...
var test_docs1 map[string]string = map[string]string{
...
var test_docs2 map[string]string = map[string]string{
...
func TestTransformGetText(t *testing.T) {
	...
	for input, expected := range test_docs1 {
...
func TestTransformGetLinks(t *testing.T) {
	...
	for input, expected := range test_docs2 {
		output, err := x.Transform(strings.NewReader(input))
		if err != nil {
			t.Error(err)
		}
